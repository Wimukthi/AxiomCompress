/* mzng_strm_split.c -- Stream for split files
...
static mzng_stream_vtbl mzng_stream_split_vtbl = {
    mzng_stream_split_open,   mzng_stream_split_is_open, mzng_stream_split_read,           mzng_stream_split_write,
    mzng_stream_split_tell,   mzng_stream_split_seek,    mzng_stream_split_close,          mzng_stream_split_error,
    mzng_stream_split_create, mzng_stream_split_delete,  mzng_stream_split_get_prop_int64, mzng_stream_split_set_prop_int64};

/***************************************************************************/

typedef struct mzng_stream_split_s {
    mzng_stream stream;
    int64_t disk_size;
    int64_t total_in;
...
#if 0
#  define mzng_stream_split_print printf
#else
#  define mzng_stream_split_print(fmt, ...)
#endif

/***************************************************************************/

static int32_t mzng_stream_split_open_disk(void *stream, int32_t number_disk) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    uint32_t magic = 0;
    int64_t position = 0;
    int32_t i = 0;
    int32_t err = MZ_OK;
    ...
    mzng_stream_split_print("Split - Goto disk - %s (disk %" PRId32 ")\n", split->path_disk, number_disk);
    ...
    if (disk_part == MZ_OPEN_MODE_READ) {
        if (strcmp(split->path_disk, split->path_cd) == 0) {
            ...
            err = mzng_os_file_exists(split->path_disk);
        }
    }
    ...
    if (err == MZ_OK) {
        ...
        if (split->mode & MZ_OPEN_MODE_WRITE) {
            ...
            if (split->current_disk == 0) {
                err = mzng_stream_read_uint32(split->stream.base, &magic);
    ...
    if (err == MZ_OK) {
        ...
        position = mzng_stream_tell(split->stream.base);
...
static int32_t mzng_stream_split_close_disk(void *stream) {
    mzng_stream_split *split = (mzng_stream_split *)stream;

    if (mzng_stream_is_open(split->stream.base) != MZ_OK)
        return MZ_OK;

    mzng_stream_split_print("Split - Close disk\n");
    return mzng_stream_close(split->stream.base);
}

static int32_t mzng_stream_split_goto_disk(void *stream, int32_t number_disk) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    int32_t err = MZ_OK;
    int32_t err_is_open = MZ_OK;

    err_is_open = mzng_stream_is_open(split->stream.base);

    if ((split->disk_size == 0) && (split->mode & MZ_OPEN_MODE_WRITE)) {
        if (err_is_open != MZ_OK)
            err = mzng_stream_split_open_disk(stream, number_disk);
    } else if ((number_disk != split->current_disk) || (err_is_open != MZ_OK)) {
        err = mzng_stream_split_close_disk(stream);
        if (err == MZ_OK) {
            err = mzng_stream_split_open_disk(stream, number_disk);
            if (err == MZ_OK)
...
int32_t mzng_stream_split_open(void *stream, const char *path, int32_t mode) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    ...
    mzng_stream_split_print("Split - Open - %s (disk %" PRId32 ")\n", split->path_cd, number_disk);
    ...
    return mzng_stream_split_goto_disk(stream, number_disk);
}

int32_t mzng_stream_split_is_open(void *stream) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    if (split->is_open != 1)
...
int32_t mzng_stream_split_read(void *stream, void *buf, int32_t size) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    ...
    err = mzng_stream_split_goto_disk(stream, split->number_disk);
    if (err != MZ_OK)
        return err;

    while (bytes_left > 0) {
        read = mzng_stream_read(split->stream.base, buf_ptr, bytes_left);

        mzng_stream_split_print("Split - Read disk - %" PRId32 "\n", read);
        ...
        if (read == 0) {
            ...
            if (size != bytes_left) /* Report read from previous disk before switching */
                break;
            err = mzng_stream_split_goto_disk(stream, split->current_disk + 1);
...
int32_t mzng_stream_split_write(void *stream, const void *buf, int32_t size) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    int64_t position = 0;
    int32_t written = 0;
    int32_t bytes_left = size;
    ...
    while (bytes_left > 0) {
        ...
        if (split->disk_size > 0) {
            if ((split->total_out_disk == split->disk_size && split->total_out > 0) ||
                ...
                err = mzng_stream_split_goto_disk(stream, number_disk);
                if (err != MZ_OK)
                    return err;
        ...
        written = mzng_stream_write(split->stream.base, buf_ptr, bytes_to_write);
        if (written != bytes_to_write)
            return MZ_WRITE_ERROR;

        mzng_stream_split_print("Split - Write disk - %" PRId32 "\n", written);
...
int64_t mzng_stream_split_tell(void *stream) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    int32_t err = MZ_OK;
    err = mzng_stream_split_goto_disk(stream, split->number_disk);
    if (err != MZ_OK)
        return err;
    return mzng_stream_tell(split->stream.base);
}

int32_t mzng_stream_split_seek(void *stream, int64_t offset, int32_t origin) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    int64_t disk_left = 0;
    int64_t position = 0;
    int32_t err = MZ_OK;
    ...
    mzng_stream_split_print("Split - Seek disk - %" PRId64 " (origin %" PRId32 ")\n", offset, origin);
    ...
    return mzng_stream_seek(split->stream.base, offset, origin);
}

int32_t mzng_stream_split_close(void *stream) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    int32_t err = MZ_OK;
...
int32_t mzng_stream_split_error(void *stream) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    return mzng_stream_error(split->stream.base);
}

int32_t mzng_stream_split_get_prop_int64(void *stream, int32_t prop, int64_t *value) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    switch (prop) {
...
int32_t mzng_stream_split_set_prop_int64(void *stream, int32_t prop, int64_t value) {
    mzng_stream_split *split = (mzng_stream_split *)stream;
    switch (prop) {
    case MZ_STREAM_PROP_DISK_NUMBER:
...
void mzng_stream_split_delete(void **stream) {
    mzng_stream_split *split = NULL;
    if (!stream)
        return;
    split = (mzng_stream_split *)*stream;
    if (split) {
        free(split->path_cd);
...
void *mzng_stream_split_get_interface(void) {
    return (void *)&mzng_stream_split_vtbl;
}
