/* mzng_os_win32.c -- System functions for Windows
...
wchar_t *mzng_os_unicode_string_create(const char *string, int32_t encoding) {
...
void mzng_os_unicode_string_delete(wchar_t **string) {
    if (string) {
        free(*string);
        *string = NULL;
    }
...
char *mzng_os_utf8_string_create(const char *string, int32_t encoding) {
    wchar_t *string_wide = NULL;
    char *string_utf8 = NULL;
    uint32_t string_utf8_size = 0;

    string_wide = mzng_os_unicode_string_create(string, encoding);
    if (string_wide) {
        ...
        mzng_os_unicode_string_delete(&string_wide);
    }

    return string_utf8;
}

char *mzng_os_utf8_string_create_from_unicode(const wchar_t *string, int32_t encoding) {
    char *string_utf8 = NULL;
    uint32_t string_utf8_size = 0;
...
void mzng_os_utf8_string_delete(char **string) {
    if (string) {
        free(*string);
        *string = NULL;
    }
...
int32_t mzng_os_rand(uint8_t *buf, int32_t size) {
...
int32_t mzng_os_path_same_fs(const char *path_a, const char *path_b) {
    ...
    path_a_wide = mzng_os_unicode_string_create(path_a, MZ_ENCODING_UTF8);
    if (!path_a_wide)
        return MZ_PARAM_ERROR;
    path_b_wide = mzng_os_unicode_string_create(path_b, MZ_ENCODING_UTF8);
    if (!path_b_wide) {
        mzng_os_unicode_string_delete(&path_a_wide);
        return MZ_PARAM_ERROR;
    }
    ...
    mzng_os_unicode_string_delete(&path_a_wide);
    mzng_os_unicode_string_delete(&path_b_wide);
    return err;
}

int32_t mzng_os_rename(const char *source_path, const char *target_path) {
    ...
    source_path_wide = mzng_os_unicode_string_create(source_path, MZ_ENCODING_UTF8);
    if (!source_path_wide) {
        err = MZ_PARAM_ERROR;
    } else {
        target_path_wide = mzng_os_unicode_string_create(target_path, MZ_ENCODING_UTF8);
        if (!target_path_wide)
            err = MZ_PARAM_ERROR;
    }
    ...
    if (target_path_wide)
        mzng_os_unicode_string_delete(&target_path_wide);
    if (source_path_wide)
        mzng_os_unicode_string_delete(&source_path_wide);

    return err;
}

int32_t mzng_os_unlink(const char *path) {
...
int32_t mzng_os_file_exists(const char *path) {
    wchar_t *path_wide = NULL;
    DWORD attribs = 0;
...
int64_t mzng_os_get_file_size(const char *path) {
    HANDLE handle = NULL;
...
static void mzng_os_file_to_unix_time(FILETIME file_time, time_t *unix_time) {
...
static void mzng_os_unix_to_file_time(time_t unix_time, FILETIME *file_time) {
    uint64_t quad_file_time = 0;
...
int32_t mzng_os_get_file_date(const char *path, time_t *modified_date, time_t *accessed_date, time_t *creation_date) {
    ...
    if (GetFileAttributesExW(path_wide, GetFileExInfoStandard, &wfad)) {
        if (modified_date)
            mzng_os_file_to_unix_time(wfad.ftLastWriteTime, modified_date);
        if (accessed_date)
            mzng_os_file_to_unix_time(wfad.ftLastAccessTime, accessed_date);
        if (creation_date)
            mzng_os_file_to_unix_time(wfad.ftCreationTime, creation_date);
...
int32_t mzng_os_set_file_date(const char *path, time_t modified_date, time_t accessed_date, time_t creation_date) {
    HANDLE handle = NULL;
    ...
    if (handle != INVALID_HANDLE_VALUE) {
        ...
        if (modified_date != 0)
            mzng_os_unix_to_file_time(modified_date, &ftm_modified);
        if (accessed_date != 0)
            mzng_os_unix_to_file_time(accessed_date, &ftm_accessed);
        if (creation_date != 0)
            mzng_os_unix_to_file_time(creation_date, &ftm_creation);
...
int32_t mzng_os_get_file_attribs(const char *path, uint32_t *attributes) {
    wchar_t *path_wide = NULL;
    int32_t err = MZ_OK;
...
int32_t mzng_os_set_file_attribs(const char *path, uint32_t attributes) {
    wchar_t *path_wide = NULL;
    int32_t err = MZ_OK;
...
int32_t mzng_os_make_dir(const char *path) {
    wchar_t *path_wide = NULL;
    int32_t err = MZ_OK;
...
DIR *mzng_os_open_dir(const char *path) {
    WIN32_FIND_DATAW find_data;
    ...
    mzng_path_append_slash(fixed_path, sizeof(fixed_path), MZ_PATH_SLASH_PLATFORM);
    mzng_path_combine(fixed_path, "*", sizeof(fixed_path));

    path_wide = mzng_os_unicode_string_create(fixed_path, MZ_ENCODING_UTF8);
    if (!path_wide)
        return NULL;
...
struct dirent *mzng_os_read_dir(DIR *dir) {
    DIR_int *dir_int;
...
int32_t mzng_os_close_dir(DIR *dir) {
    DIR_int *dir_int;
...
int32_t mzng_os_is_dir_separator(char c) {
    return c == '\\' || c == '/';
}

int32_t mzng_os_is_dir(const char *path) {
    wchar_t *path_wide = NULL;
    uint32_t attribs = 0;
...
int32_t mzng_os_is_symlink(const char *path) {
    wchar_t *path_wide = NULL;
    uint32_t attribs = 0;
...
int32_t mzng_os_get_link_attribs(const char *path, uint32_t *attributes) {
    wchar_t *path_wide = NULL;
    int32_t err = MZ_OK;
...
int32_t mzng_os_make_symlink(const char *path, const char *target_path) {
    ...
    target_path_wide = mzng_os_unicode_string_create(target_path, MZ_ENCODING_UTF8);
    if (target_path_wide) {
        if (mzng_path_has_slash(target_path) == MZ_OK)
...
int32_t mzng_os_read_symlink(const char *path, char *target_path, int32_t max_target_path) {
    ...
    if (DeviceIoControl(handle, FSCTL_GET_REPARSE_POINT, NULL, 0, buffer, sizeof(buffer), &length, NULL) == TRUE) {
        ...
        if ((IsReparseTagMicrosoft(reparse_data->ReparseTag)) && (reparse_data->ReparseTag == IO_REPARSE_TAG_SYMLINK)) {
            ...
            if (target_path_wide) {
                ...
                target_path_utf8 = mzng_os_utf8_string_create_from_unicode(target_path_wide, MZ_ENCODING_UTF8);
                ...
                if (target_path_utf8) {
                    ...
                    if (mzng_os_is_dir(target_path_utf8) == MZ_OK)
                        mzng_path_append_slash(target_path, max_target_path, MZ_PATH_SLASH_PLATFORM);
                    mzng_os_utf8_string_delete(&target_path_utf8);
                } else {
                    err = MZ_MEM_ERROR;
                }
...
int32_t mzng_os_get_temp_path(char *path, int32_t max_path, const char *prefix) {
    ...
    if (prefix) {
        prefix_wide = mzng_os_unicode_string_create(prefix, MZ_ENCODING_UTF8);
        if (!prefix_wide)
            return MZ_MEM_ERROR;
    }

    if (GetTempFileNameW(tmp_dir_wide, prefix_wide ? prefix_wide : L"", 0, tmp_path_wide) == 0) {
        mzng_os_unicode_string_delete(&prefix_wide);
        return MZ_INTERNAL_ERROR;
    }
