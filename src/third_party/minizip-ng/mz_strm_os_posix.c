/* mzng_strm_posix.c -- Stream for filesystem access for posix/linux
...
typedef struct mzng_stream_posix_s {
    mzng_stream stream;
    int32_t error;
    FILE *handle;
} mzng_stream_posix;
...
int32_t mzng_stream_os_open(void *stream, const char *path, int32_t mode) {
    mzng_stream_posix *posix = (mzng_stream_posix *)stream;
...
void *mzng_stream_os_create(void) {
    mzng_stream_posix *posix = (mzng_stream_posix *)calloc(1, sizeof(mzng_stream_posix));
    if (posix)
        posix->stream.vtbl = &mzng_stream_os_vtbl;
    return posix;
}

void mzng_stream_os_delete(void **stream) {
    mzng_stream_posix *posix = NULL;
    if (!stream)
        return;
    posix = (mzng_stream_posix *)*stream;
    free(posix);
    *stream = NULL;
}
