# The Axiom archive format

This specifies the on-disk layout of a multi-file Axiom archive (`.axar`) and
of the single-stream `.axc` payload it embeds once per solid block.
...
sizes are absolute byte positions in the archive file, unless a field says
otherwise.

**The documented limits and validation rules are part of the format.**
Rejecting malformed data is intended behaviour, not an implementation
detail — a reader that accepts an out-of-range value is not being lenient, it
is being wrong.
...
bytes live. An optional generation record links an appended update back to the
previous footer. Optional recovery data protects against damage. The footer
points back to the directory, which is how a reader finds everything without
scanning.
...
- Compress with cross-file redundancy — files are grouped into *solid blocks* —
  while keeping each block **independently decompressible**, so selective
  extraction and bounded-memory decode both stay possible.
- Write in a single pass with bounded memory: one ordinary solid block in
  memory at a time. The large LZMA2 profile uses a temporary-file spool and
  bounded codec chunks instead.
- Localize integrity failures: per-block checks from the embedded `.axc`, plus
...
the thread count. That is writer policy, not a different on-disk layout — a
reader never needs to know how the size was picked.
...
a self-contained `.axc` payload with its own header and CRC-32. That CRC is the
...
profile currently requires LZMA2.

The writer stages the raw block in a temporary file and emits one AXC v10
external-codec envelope whose LZMA2 chunks are **512 MiB by default**, or up to
**4 GiB−1** when a larger dictionary is explicitly requested. The outer
solid-block size and the codec chunk size are separate limits: a 64 GiB solid
block does not require a 64 GiB allocation or a 64 GiB LZMA2 dictionary.

Every large block carries a complete external-chunk subframe map. Readers use
that map for extraction and testing, decoding only the chunk ranges the current
file operation needs. **A large block without a valid map is rejected.**
...
records are requested — those paths still use whole-block buffers and need
their own streaming profiles before they can combine. Older readers reject flag
`0x0040` before they ever parse the directory.
...
During an add, update, fresh, or sync operation, a regular file whose size,
CRC-32, and BLAKE3 digest match an existing stored file may reuse that file's
data range. Repack performs the same coalescing for duplicate entries while it
rewrites blocks.

This is represented entirely by ordinary directory addresses. It adds no flag
and requires no new AXAR version. If the source cannot be hashed reliably, the
writer takes the normal new-block path instead.
...
Ranges are ordered, non-overlapping, and bounded by `original_size`. The
independent source offsets are what let files and file fragments reset a filter
safely when several inputs share a solid block, or when one file spans blocks.
...
out-of-range, or implausibly numerous ranges are rejected before any decode
allocation happens.
...
**Version 6** stores the sequence count and the final literal count, followed by
...
and adds two static literal modes: match-byte (the first literal after a match,
...
recent-distance table; value `4` reads one explicit distance from the normal
distance-slot stream. The four selected values replace the table atomically.
Checkpoint distances must be nonzero and no greater than the output position.
Descriptor, stream-consumption, and footer padding checks remain exact. Matches
retain the full block window but may not cross the static encoder-selected
...
requires the backend to produce that exact size and to consume the complete
encoded chunk, and rejects trailing bytes.

Writers store a chunk verbatim when the selected codec would expand it.

LZMA2 property `40` denotes the maximum 4 GiB−1 dictionary. The effective
dictionary is bounded by the declared chunk and by the stable input bound
chosen for the payload: ordinary payloads use their total input size, while the
disk-streamed large-solid profile uses its working-chunk bound. That is what
keeps a small payload from forcing a multi-gigabyte allocation while letting
every frame retain one property byte.

The chunk boundary is also the cooperative pause, cancel, and progress
boundary. Transform metadata, CRC-32, encryption, recovery, signing, and all
AXAR directory semantics are unchanged by this envelope.

## Central directory

Located at `directory_offset`. The directory is **vint-encoded** — LEB128
unsigned varints, 7 bits per byte, with the high bit meaning "more bytes
follow" — and **extensible**: every record that can grow carries a length, so a
reader consumes the optional fields it knows and skips the rest.
...
Archive-wide service data. Unknown types are skipped by their length.
...
The current optional block-extra profile is type `1`, the **subframe map**. It
is not a new AXAR version and not a required header flag. Writers omit it when
a block is encrypted, transformed, or has no independently decodable frames.
Its payload is:
...
The map ranges are sorted, non-empty, and cover the complete uncompressed solid
block without gaps. Compressed ranges are bounded and non-overlapping. A stored
AXC block has one range; for the parallel-block and external-codec envelopes,
each range points at the complete frame or chunk header as well as its payload.
...
A file's bytes are recovered by reading `size` bytes starting at
...
Multiple file entries may intentionally share the same (`first_block`,
...
Content mutation through ordinary add/update/fresh/sync/delete/move operations
is rejected, so a legacy operation cannot silently discard snapshot history.
Metadata-only operations such as comments, locking, password-slot changes, and
signing keep the chunk table and manifests intact.

### Sparse-file allocation maps

AXAR always stores the complete logical byte stream, including the zero bytes
inside holes. When the source filesystem exposes reliable allocated ranges, the
writer adds entry extra type 8 and upgrades the header to v5.

Doing it this way keeps compression, hashing, and old v4 directory addressing
straightforward, while still allowing a restore to recreate the source file's
physical allocation pattern.

The extent list is sorted, non-overlapping, non-empty, and bounded by the
entry's logical `size`. An empty list is valid: it represents a file whose
entire logical range is a hole.

If a sparse candidate cannot be queried, the writer archives the bytes densely,
records the reason in archive extra type 5, and reports it through the
operation warning channel. Strict metadata mode turns that warning into a
failed operation. Extraction similarly reports an inability to punch holes,
unless strict metadata mode is enabled.

## Append generations

An archive update can append a new generation without copying the previous
block region. The previous complete archive stays byte-for-byte at the start of
the file, including its directory, optional recovery data, and legacy footer.
The new generation appends any new solid blocks, a new directory, this
extension, optional recovery data, and a new legacy footer.

Replaced data is therefore retained as dead space until `repack` or another
compacting rewrite reclaims it. That is the deliberate trade: a completed
update never depends on rewriting the whole file.

The original v4/v5 layout without this record is the legacy starting
generation. The first appended generation is numbered `1`, and later
generations increment that number. The fixed extension sits immediately after
the current directory and is always exactly 64 bytes:
...
present: `[0, directory_offset + directory_size + 64)`.

The final footer keeps the legacy 24-byte shape, so a reader that only
understands AXAR v4/v5 can still locate the current directory in an intact
appended archive. A generation-aware reader additionally scans backwards for
the newest valid footer when a write was interrupted after the previous footer;
an incomplete or invalid newer extension is ignored, and the previous complete
generation is opened instead.
...
the old signature before writing the replacement directory.

Direct append is used when header features remain compatible. Header-changing
operations, compaction, deletion, and anything else needing a complete rebuild
retain the atomic temporary-file replacement path.

## Encryption

When an archive is created with a password, every solid block is encrypted.
AXAR has two encryption profiles: legacy v1, retained for AXAR v4
compatibility, and encryption-v2, used for new password archives. **Readers
must support both.**

### Common block encryption

- The password is never stored.
- Each compressed solid block is sealed independently with
  XChaCha20-Poly1305.
- A block's index is the AEAD associated data, which prevents block reordering
  and cross-archive transplants.
- Block-only encryption leaves the central directory readable.
  `--encrypt-names` also seals the directory, so listing needs a password.
...
`Argon2id(password, salt, params)` derives the 32-byte archive key. The same key
seals the solid blocks and, when requested, the directory. The `key_check` value
is a known plaintext sealed under that key; readers authenticate it before
reading block data, so a wrong password is rejected early rather than after a
long failed decode.
...
XChaCha20-Poly1305. The key identifier and the slot identifier are authenticated
associated data.

The consequence is the useful part: any slot password unlocks the same archive,
and changing one password replaces only that slot's wrapper. No compressed
bytes move.

**Encrypted directory.** For an encrypted directory, the same payload is stored
in a plaintext preamble immediately after the 16-byte header: a little-endian
`u32` payload length followed by the type-6 payload. The directory is then
sealed as `nonce(24) ‖ tag(16) ‖ ciphertext` with the fixed directory
associated-data tag. In this form there is no type-6 directory record, because
the directory is not readable until the preamble has been processed. Header
flags `0x0001` and `0x0010` are both set.

**Blocks.** Each block's compressed `.axc` bytes are sealed under the random
archive data key. `compressed_size` covers the complete
`nonce(24) ‖ tag(16) ‖ ciphertext` blob; `uncompressed_size` remains the
...
duplicate slot IDs, unsupported options, and implausible KDF costs are all
rejected before any expensive key derivation or archive write.
...
`password-add` appends a slot, `password-change` replaces the slot identified by
the current password, and `password-remove` removes the slot identified by the
final password. The last remaining slot cannot be removed.

A password mutation authenticates the current password first, then rewrites only
the encryption metadata and the directory. Existing compressed and encrypted
block bytes are copied verbatim — never decompressed, recompressed, or
re-encrypted. For an encrypted directory, the preamble and directory are
rewritten and block offsets may move, but the stored block payloads themselves
are untouched.
...
archives remain readable without ever migrating.

Password mutations remove an existing archive signature, because the signed
password metadata has changed; re-sign afterwards. Any existing recovery service
is rebuilt against the new header and directory layout.
...
The v2 preamble stays public because it contains only salts, KDF costs, slot
identifiers, and wrapped keys. It does not contain the password or the plaintext
archive key.

Both legacy v1 and v2 encrypted-directory archives can be listed, tested,
extracted, edited, signed, and verified when the correct password is supplied.

## Recovery service

An archive created with `--recovery N`, or updated with `axiomc recovery`,
places a systematic Reed-Solomon service after the central directory.

For a legacy layout the protected range is
`[0, directory_offset + directory_size)`. For an appended generation it is
...
normal footer stays last, so listing and extraction need no special path when
the archive is intact.

Repair validates each shard CRC, treats failures as erasures, reconstructs up to
...
not parseable archives on their own.

For `name.axar`, data volumes are `name.part001.axar`, `name.part002.axar`, …
and optional recovery volumes are `name.rev001`, `name.rev002`, … . Every member
begins with this 80-byte header:
...
Joining validates set identity and payload CRCs, reconstructs unavailable shards
when enough members survive, truncates to `archive_size`, verifies
...
temporary joined file, and the set stays read-only. Joining is required only for
...
generation extension or a recovery service precedes it.
...
seeks to `directory_offset`. No scanning of the blocks is ever required.

Generation-aware readers additionally scan backwards for the newest valid footer
if the physical tail is longer than a complete generation, which is what an
interrupted append looks like. The older footer remains usable as the crash
fallback.

## Resource limits

Declared sizes in the header, the directory, and the embedded `.axc` blocks are
**untrusted**. An overlapping match can expand a tiny payload to fill an
arbitrary declared size, so the decoder defends itself before allocating:
...
- `decompress` rejects any stream whose declared original size exceeds a
  ...
  by that limit rather than by whatever the header claims.
...
- Native Axiom match distances are 32-bit, so the full-window path supports a
  ...
  ceiling. The external LZMA2 large-block profile raises the AXAR grouping limit
  to **64 GiB** while keeping its default independently decoded codec chunk at
  **512 MiB**. Total archive size across blocks is `u64`.
- The level-1 `fast_lz` block codec: match **distance ≤ 16 MiB** (24-bit),
  match **length ≤ 273**.
- Stored **path length ≤ 65,535 bytes**.
- File and block counts are `u64`, but bounded in practice by memory.
...
the ordinary block-size ceiling. AXC v10 readers predating the 4 GiB LZMA2
geometry may likewise reject an archive whose declared LZMA2 chunk exceeds their
older 512 MiB safety bound; existing v4/v5 archives are not affected.

An appended generation does not change the AXAR header version. Readers that
don't understand its 64-byte extension can still read the current directory from
the final legacy footer of an intact archive — they just don't get generation
history or the generation-aware crash fallback.
...
- **Solid compression** with per-block and per-file CRC-32, random-access
  ...
  `--overwrite fail|skip|all`, mtime restore, and threaded encode and decode.
- **Seekable selected extraction** — new plaintext AXAR blocks may carry a
  skippable subframe map. Selected restores fetch and decode only the
  intersecting independent frames; legacy, encrypted, transformed, and serial
  blocks use the ordinary whole-block path. Operation telemetry exposes the
  physical archive bytes read, including directory reads.
- **Sparse-file fidelity** — logical bytes stay fully checksummed while
  supported files preserve their allocated ranges on extraction. The archive
  exposes an explicit capture report for skipped or unavailable source metadata,
  and strict metadata mode is available to callers that require lossless
  capture.
- **Add and update into an existing archive** — existing files are not
  ...
  the directory is rebuilt. When the header feature set stays compatible, the
  ...
  entry of the same name, and the replaced bytes become dead space until a
  repack.
- **File-manager operations** — map a filesystem file or directory to an
  ...
  recompressing block data. Hard-link targets are rewritten when their canonical
  path moves; a selectively extracted hardlink is materialized as a regular file
  when its canonical entry wasn't selected.
...
- **Comment and lock** — a free-form UTF-8 comment and a one-way read-only flag.
  Both live in the archive-level TLV and survive edits; reads ignore the lock.
- **Encryption** — per-block XChaCha20-Poly1305 with an Argon2id password key.
- **Signatures** — archive metadata type 4 stores a Curve25519 EdDSA public key
  and signature using Monocypher's BLAKE2b-based primitive. The signed digest
  covers the exact header, preamble, and block bytes plus canonical directory
  semantics. `test` rejects an invalid signature, and any edit removes the stale
  signature. This primitive is **not** wire-compatible with standard SHA-512
  Ed25519.
- **SFX packaging** — a 64-byte descriptor and an intact `.axar` or `.zip` are
  ...
  afterwards without displacing it. Because Authenticode covers trailing data,
  signing a packaged SFX authenticates the payload as well as the stub. Readers
  fall back to the pre-0.8.0.0 layout, an `"AXIOMSFX"` magic and `u64` length in
  the last 16 bytes, so existing executables keep working. The image ships as
  the non-executable `AxiomSfx.bin` and is read only during SFX creation. This
  wrapper changes neither archive format.
...
- **Sequential AXAR v5 output** to a non-seekable stream. The writer reserves
  the known required-feature flags up front and emits a counted directory and
  footer without seeking. Recovery records and streaming-time signatures are not
  part of this profile.

### Metadata

mtime in seconds, plus — on Windows — file attributes, full-precision
...
restored best-effort on POSIX hosts; restoring ownership requires privilege.
Sparse allocation maps and incomplete-capture warnings are v5 metadata, and can
be queried through the library's archive capture-report API.
...
unsupported or failed restore is reported — or fails outright, in strict
metadata mode.
...
- **No arbitrary in-place mutation.** Append-compatible updates retain prior
  ...
  changes, and other rebuilds still use a temporary file plus an atomic rename.
  Recovery and signing are file-output services, unavailable on the
  non-seekable stream profile.
...
It is also **symlink-safe**, which matters more. Lexical containment only proves
a path *spells* no escape. So before materializing any entry, the extractor
requires every existing directory component — from the destination root down to
that entry's parent — to be a real directory and not a redirecting link. This
covers symlinks on every platform and, on Windows, **NTFS junctions and mount
